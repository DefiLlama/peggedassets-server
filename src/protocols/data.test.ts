...
const protocolsThatCantBeImported = [
  "Genshiro",
  "Shiden dApp Staking",
  "Astar dApps Staking",
];
async function importProtocol(protocol: Protocol) {
  if (
    protocol.name.startsWith("Karura ") ||
    protocolsThatCantBeImported.includes(protocol.name)
  ) {
    return {};
  } else {
    ...
    Object.entries(module).map((entry) => {
      if (!ignored.includes(entry[0]) && typeof entry[1] === "object") {
        ...
        if (chainCoingeckoIds[chain] === undefined) {
          throw new Error(
            `${chain} (found in ${protocol.name}) should be on chainMap`
          );
        }
      }
    });
