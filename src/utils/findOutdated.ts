...
function printOutdated(
  outdated: [string, InfoProtocol, boolean][],
  maxLengthProtocolName: number,
  now: number
) {
  return outdated
    .sort((a, b) => {
    ...
    .map((line) => {
      ...
      if (line[1] === null) {
        ...
        text = `Last update: ${new Date(
          line[1].time * 1000
        ).toDateString()} ${humanizeTimeDifference(
          now - line[1].time
        )} - ${humanizeNumber(line[1].tvl)}`;
      }
      return `${line[0]} - ${text}`;
    })
    .join("\n");
...
export default async function findOutdated(maxDrift: number) {
  ...
  const maxLengthProtocolName = outdated.reduce(
    (max, line) => Math.max(max, line[0].length),
    0
  );
  return `REFILLABLE
${printOutdated(
  outdated.filter((p) => p[2]),
  maxLengthProtocolName,
  now
)}
