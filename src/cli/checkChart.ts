  ...
  for (const tvlPrefix of [dailyTvl, dailyTokensTvl, dailyUsdTokensTvl]) {
    ...
    await Promise.all(
      protocols.map(async (protocol) => {
        ...
        if (
          historicalTvl.Items !== undefined &&
          historicalTvl.Items.length > 0
        ) {
          const items = historicalTvl.Items;
          let lastItemDate = getClosestDayStartTimestamp(
            items[items.length - 1].SK
          );
          for (let i = items.length - 2; i >= 0; i--) {
            ...
            if (!notify && over && protocolIsSelected) {
              ...
              do {
                ...
                if (extrapolate) {
                  throw new Error(
                    "Extrapolation is bad, removign this error if you want to continue anyway"
                  );
